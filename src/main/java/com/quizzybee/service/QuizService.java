...
public class QuizService {
    ...
    public Quiz generateQuiz(String syllabusText, String apiKey) throws IOException {
        return generateQuiz(syllabusText, apiKey, 10); // Default to 10 questions
    }

    /**
     * Generates a quiz with MCQs based on the provided syllabus text with a
     ...
     * @throws IOException If there's an error communicating with the API
     */
    public Quiz generateQuiz(String syllabusText, String apiKey, int numQuestions) throws IOException {
        ...
        String prompt = createGeminiPrompt(syllabusText, numQuestions);
    ...
    private String createGeminiPrompt(String syllabusText) {
        return createGeminiPrompt(syllabusText, 10); // Default to 10 questions
    }

    /**
     * Creates a prompt for the Gemini API requesting MCQ generation with a
     * specified number of questions
     */
    private String createGeminiPrompt(String syllabusText, int numQuestions) {
        return "Based on the following syllabus content, generate " + numQuestions
    ...
    private void parseJsonQuestions(String json, Quiz quiz) {
        try {
            ...
            for (JsonNode questionNode : questionsArray) {
                String questionText = questionNode.has("question") ? questionNode.get("question").asText() : "";
                ...
                int correctIndex = questionNode.has("correctOptionIndex")
                        ? questionNode.get("correctOptionIndex").asInt()
                        : 0;
    ...
    private void fallbackParseResponse(String response, Quiz quiz) {
        ...
        for (String block : questionBlocks) {
            ...
            try {
                ...
                if (lines.length < 5)
                    continue; // Need at least question + 4 options
